...
if __name__ == "__main__":
    ...
    for i in range(4, 10):
        ...
        y1 =y1.astype(int).reshape(x1.shape[0], 1)
        z1 =z1.astype(int).reshape(x1.shape[0], 1)
        
        t = np.concatenate((x1,y1,z1), axis=1)
        
        unique, counts = np.unique(t, return_counts=True, axis=0)
        
        tt = unique[counts>thresh]
        x1 = tt[:, 0]
        y1 = tt[:, 1]
        z1 = tt[:, 2]
        

        #tp = np.stack((x1, y1), axis=-1)
        #tp1 = np.stack((tp, z1), axis=2)
        ...
        shell = mayavi.mlab.points3d(-y1, -z1, x1, mode="cube", scale_factor=0.8, color=(0, 0, 1))
