...
legPos = [[height / 2 + outX/3, width / 2 + turnLineDist],
          [height / 2 + outX/3, -width / 2 - turnLineDist],
          [-height / 2 - outX/3, width / 2 + turnLineDist],
          [-height / 2 - outX/3, -width / 2 - turnLineDist]]  # initial positions of legs (global)
...
turnRelLeg = [[outX/3, turnLineDist, -turnHeight],
              [outX/3, turnLineDist, -turnHeight],
              [-outX/3, turnLineDist, -turnHeight],
              [-outX/3, turnLineDist, -turnHeight]]
...
def stepLegs(curRelPos, newRelPos):

    curRelPos = copy.deepcopy(curRelPos)
    newRelPos = copy.deepcopy(newRelPos)

    for leg in range(4):
        execInst(inst(inst_type.abs, curRelPos[leg]), leg)
    ...
    for moveLeg in order:
        ...
        for leg in range(4):  # move back and tilt
            if leg == moveLeg:
                execInst(inst(inst_type.abs, [curRelPos[leg][0], curRelPos[leg][1], -turnHeight - tiltHeight]), leg)
        ...
        for leg in range(4):  # move back and tilt
            if leg == moveLeg:
                execInst(inst(inst_type.abs, [curRelPos[leg][0], curRelPos[leg][1], -turnHeight + liftHeight]), leg)
            elif leg == opposites[moveLeg]:
                execInst(inst(inst_type.abs, [curRelPos[leg][0], curRelPos[leg][1], -turnHeight + tiltHeight]), leg)
            else:
                execInst(inst(inst_type.abs, curRelPos[leg]), leg)
...
def mainLoop():
    ...
    for ii in range(0, amTurn+1, step):
        rot = np.deg2rad(ii)
        ...
        for j in range(len(legPos)):
            ...
            execInst(inst(inst_type.abs, globToLoc(legPos[j], bodyCorners[j], 0, j, -turnHeight)), j)
