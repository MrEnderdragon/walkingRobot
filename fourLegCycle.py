...
timePerCycle = 3 # amount of time taken for every cycle (seconds)
...
# lift, move, move, move
# 3 steps per "move", 3 lift + 9 move
moveCycle = [
[-maxX,lineDist,-liftHeight+60], # back lift  \
[maxX,lineDist,-liftHeight+60], # front lift   }  LIFT
[maxX,lineDist,-liftHeight], # front drop    /

[7/9*maxX,lineDist,-liftHeight], #  \
[5/9*maxX,lineDist,-liftHeight], #  }  MOVE
[3/9*maxX,lineDist,-liftHeight], # /

[1/9*maxX,lineDist,-liftHeight], #   \
[-1/9*maxX,lineDist,-liftHeight], #  }  MOVE
[-3/9*maxX,lineDist,-liftHeight], # /

[-5/9*maxX,lineDist,-liftHeight], #  \
[-7/9*maxX,lineDist,-liftHeight], #  }  MOVE
[-maxX,lineDist,-liftHeight], #     /
]
